...
export const normalizeInvoiceFromResponse = (response: GetInvoiceDetailsResponse): Invoice => {
  return {
    ...
    excelDocumentDate:
      typeof response.ExcelDocumentDate === 'string' ? parseDate(response.ExcelDocumentDate) : undefined,
  };
};
...
export const normalizeInvoiceListItemFromResponseItem = (response: GetInvoiceOverviewResponseItem): InvoiceListItem => {
  return {
    ...
    excelDocumentDate:
      typeof response.ExcelDocumentDate === 'string'
        ? parseDate(response.ExcelDocumentDate, "yyyy-MM-dd'T'HH:mm:ss")
        : undefined,
  };
};
