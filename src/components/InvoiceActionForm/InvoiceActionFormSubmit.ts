...
export type onInvoiceSuccessFullHandle = () => void;
export type onInvoiceSubmitParams = {
  onSuccess?: onInvoiceSuccessFullHandle;
};
...
export const useInvoiceActionFormSubmit = (invoiceId: string) => {
  ...
  const submit = useCallback(
    async ({ onSuccess }: onInvoiceSubmitParams) => {
      if (!item) {
        return;
      }
      ...
      try {
        ...
        const isSuccessfull = getResultKey(result) === 'DOCUMENT_SAVED_SUCCESSFULLY';
        ...
        await invalidateInvoice(invoiceId);
        await refetchInvoice(); // refetch invoice

        // call sucessfull function
        if (isSuccessfull && onSuccess) {
          onSuccess();
        }
      } catch (err) {
